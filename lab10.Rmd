...
```{sql, connection=con}
SELECT MAX(amount) AS max,MIN(amount) AS min,AVG(amount) AS avg,SUM(amount) AS sum
FROM payment
```
...
```{sql, connection=con}
SELECT customer_id, MAX(amount) AS max,MIN(amount) AS min,AVG(amount) AS avg,SUM(amount) AS sum, COUNT(*) AS count
FROM payment
GROUP BY customer_id
HAVING count>5
```
